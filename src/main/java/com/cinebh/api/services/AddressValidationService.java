...
public class AddressValidationService {
    private static final String BOSNIA_AND_HERZEGOVINA_COUNTRY_CODE = "ba";
    ...
    public boolean isValidStreetInCity(final String city, final String streetAddress) {
        ...
        if (!nominatimProperties.enabled()) {
            log.warn("Nominatim validation is disabled.");
            return false;
        }
    ...
    private boolean validateWithNominatim(
        ...
        try {
            ...
            log.warn(
                    "Nominatim validation failed for city={} streetName={}",
                    city,
                    streetName,
                    exception
            );
    ...
    private List<NominatimSearchResponse> searchFreeForm(
        ...
        final List<NominatimSearchResponse> results = nominatimRestClient.get()
                .uri(uriBuilder -> uriBuilder
                        ...
                        .queryParam("layer", "address")
                        .queryParam("countrycodes", nominatimProperties.countryCodes())
                        .queryParam("q", String.format("%s, %s, Bosnia and Herzegovina", streetName, city))
                        .build())
                .retrieve()
                .body(new ParameterizedTypeReference<>() {
                });
    ...
    private boolean isBosniaAndHerzegovina(final NominatimSearchResponse result) {
        final Map<String, String> address = result.address();
        if (address == null) {
            return false;
        }
        return BOSNIA_AND_HERZEGOVINA_COUNTRY_CODE.equalsIgnoreCase(
                address.getOrDefault("country_code", "")
        );
    }
